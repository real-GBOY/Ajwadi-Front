...
import { useGetUserById, useVerifyAsFreelancer, useUnverifyAsFreelancer } from '../hooks/users/useUsers';
import {
  useGetUserIdentity,
  useGetUserProposals,
  useGetUserContracts,
...
export default function FreelancerDetailPage() {
  ...
  const verifyAsFreelancerMutation = useVerifyAsFreelancer();
  const unverifyAsFreelancerMutation = useUnverifyAsFreelancer();
    ...
    return (
      <div className="p-8">
        <div className="bg-white rounded-lg border border-border p-6">
          <p className="text-text-sub">المستقل غير موجود</p>
        </div>
      </div>
    );
  }
  ...
  const handleToggleFreelancerVerification = async () => {
    if (!id) return;
    try {
      if (user.isVerifiedAsFreelancer) {
        await unverifyAsFreelancerMutation.mutateAsync(id);
        setSuccessMessage('تم إلغاء التحقق كمستقل بنجاح');
      } else {
        await verifyAsFreelancerMutation.mutateAsync(id);
        setSuccessMessage('تم التحقق كمستقل بنجاح');
      }
      refetch();
      setSuccessModalOpen(true);
    } catch (error) {
      console.error('Error toggling freelancer verification:', error);
    }
  };
                ...
                {contractsData.map((contract) => (
                  <div
                    ...
                    <div className="flex items-center gap-4 text-xs text-text-sub">
                      ...
                      {contract.clientData && (
                        <span>العميل: {contract.clientData.name}</span>
                      )}
                    </div>
                  </div>
                ))}
            ...
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <div className="flex items-center gap-4 flex-wrap">
                  ...
                  {user.jobTitle && (
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-text-sub">المسمى الوظيفي:</span>
                      <span className="text-sm text-text-strong">{user.jobTitle}</span>
                    </div>
                  )}
                  {user.specification && (
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-text-sub">المجال:</span>
                      <span className="text-sm text-text-strong">{user.specification.name}</span>
                    </div>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={handleToggleFreelancerVerification}
                  disabled={
                    verifyAsFreelancerMutation.isPending ||
                    unverifyAsFreelancerMutation.isPending
                  }
                  ...
                  {user.isVerifiedAsFreelancer ? (
                    <>
                      ...
                      <span>إلغاء التحقق كمستقل</span>
                    </>
                  ) : (
                    <>
                      <CheckCircle className="w-4 h-4" />
                      <span>التحقق كمستقل</span>
                    </>
                  )}
                </button>
                <button
